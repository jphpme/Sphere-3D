...
| `src/ui/chatUI.ts` | Orbit chat panel — rendering, settings, trigger positioning; the §A6 `show-analysis` chip that opens Analyze on a region Orbit just measured (display-only, never deferred, and absent unless the host wires `onShowAnalysis`). Also the immersive voice seam (`toggleImmersiveVoice` / `getImmersiveVoiceState` / `endImmersiveVoice`, ORBIT_VOICE_PLAN §5.4) the VR/AR HUD drives when the panel isn't visible: such a turn is spoken regardless of auto-speak and carries out the reply's first Load, and its state is cached so the per-XR-frame poll does no work |
| `src/ui/mapLayersUI.ts` | AYNI — the Layers section of the Tools menu (basemap, overlays, overlay colour), rendered into the empty `#tools-menu-layers` section toolsMenuUI leaves for it; hidden while the catalog has no layers |
...
| `src/ui/vrRotateTouch.ts` | Post-placement touch input for handheld AR (an Android phone or tablet), and deliberately the smallest thing that works: **one finger dragged sideways spins the globe on its own axis, and touch does nothing else.** A second finger is ignored outright — no pinch, no twist, no move — and no touch on this layer can write the globe's scale or position; sizing is the explicit zoom slider, placement is `vrPlacementTouch`. It replaced a one-finger-move / two-finger-pinch / two-finger-twist layer under which every stray contact did something to the globe, which on a phone read as "any press resizes it". DOM-driven because a screen tap's XR ray starts at the *device* and points through the finger, so a drag cannot be read as finger travel through the XR path; `vrInteraction` declines its own globe grab while this is mounted (`domTouchActive`). Nothing is `preventDefault()`ed until the finger has travelled past a 10 px dead zone, so plain taps still reach the XR path for the HUD, browse panel and tour strip. Headsets never mount it. Which device gets it is decided once per session by `isHandheldArUserAgent` in `vrCapability`, never from the input sources — see that helper for why. AYNI: the same drag's vertical travel tilts the globe toward or away from the viewer (`onTilt`, applied by vrGlobeTilt); a second finger still does nothing |
| `src/services/vrGlobeTilt.ts` | AYNI — `tiltGlobe`: tilts the immersive globe about the horizontal line across the viewer's line of sight (the camera's right vector, levelled), in the globe's parent frame so a placed globe under a rotated anchor behaves, and before the globe's own rotation so a later spin stays about its tilted polar axis. Bounded at 80° from upright: moves further are refused, moves back always allowed |
| `src/ui/vrPlacementTouch.ts` | Placement chrome for handheld AR (dom-overlay sessions) — the DOM Place / Cancel / Re-place buttons and the step hint, and nothing else: the layer owns no gesture. Interception is the other half: while Place mode is active it `preventDefault()`s every `beforexrselect` reaching the overlay root, so a stray screen tap never reaches the XR `select` path and confirms a placement the user did not ask for. It used to own a vertical drag that set the height; the height step is the pose's own pitch on every device now (a headset's head, a phone's own aim — see `vrSession`), and the phone's touch is rotate-only (`vrRotateTouch.ts`). |
| `src/services/dsaTimeline.ts` | The time axis a real-time or forecast stream **declares beside itself**, read from the `.dsa` the R2 publisher writes next to `stream.mpd`. Those catalog rows carry no `startTime`/`endTime`, so `utils/time.ts`'s inferred display interval has nothing to work from while this *declares* the axis — and because the publisher encodes one data frame per encoded video frame the mapping is arithmetic: `frame = floor(videoTime × videoFrameRate)`, `date(frame) = timeRange.start + frame × timeCadenceSeconds`, duration `= timeTotalFrames / videoFrameRate`. Verified against live files (2,880 frames at 900 s / 24 fps, 834 at 14,400 s, 96 at 54,000 s, a regional forecast at 31 / 3,600 s encoded at **8** fps) and against the MPD from the other side (`mediaPresentationDuration="PT2M0.2S"` for 2880/24, `availabilityStartTime` equal to `timeRange.start`). Parsing is deliberately strict and returns null rather than a partial axis — not a series, no origin, no frame count, no cadence (derivable from the range when absent), or no frame rate all mean "no dates", because a wrong date on a forecast is indistinguishable from a right one. `endMode` (`exclusive`/`last_frame`) is kept for honesty while the maths goes through `frameCount × cadence`; a seek lands **mid-frame** (`(frame + 0.5) / fps`) so a decode cannot resolve to the neighbouring date; and `dataAvailability.spans` is sparse — an unlisted frame is `real` when the status says complete/partial, `unknown` when there is no block at all. Pure arithmetic: no DOM, no Three.js, no fetch. |
| `src/services/dsaTimelineCache.ts` | Fetch-and-remember for those axes, shaped for a render loop: `prefetch(url)` is fire-and-forget, `get(url)` is a map read, neither throws, and a per-frame poll may call both unconditionally. Three rules, each a bug the other way round: **one fetch per URL, success or failure** (one index row's `.dsa` 404s today, and retrying a miss every frame would put ninety failing requests a second on the wire), **concurrent callers share the request**, and **a missing axis is not an error** — `get` returns null for "not arrived", "no time block" and "no such file" alike, and the caller draws no track. `fetchImpl` is injected, and `undefined` (use the global fetch) is deliberately distinct from `null` (there is none) — a distinction `??` folds together, which is how a test for the offline path ended up reaching the network. Also keeps each descriptor's metadata (`getMetadata` / `loadMetadata`), parsed from the same response as the axis, so Orbit reading a stream costs no second request |
| `src/services/dashRelease.ts` | AYNI — value-encoded DASH releases, on the browser globe and the immersive one. The stream host publishes some streams as a `latest.json` pointer to an immutable `release.json` whose frames carry measurements in luma (`luma8-linear` / `luma8-log` / `luma8-classified`) above a calibration strip. `resolveDashRelease` follows the pointer to the current MPD, `.dsa` and encoding at every load; `parseReleaseEncoding` reads the publisher's code layout (no data below `nodataThresholdCode`, palette across `dataMinCode`..`dataMaxCode`, classified codes snapped to the nearest class) and presentation (`gradient` / `binary` / `opaque` alpha); `buildReleaseLut` is the exact 256-entry palette, and `releaseColorScale` restates it one stop per code so both globes' existing data-encoded branches reproduce it entry for entry; `valueAtCode` is the decoder the value readout uses; `releaseCropRect` (image space, the browser globe's `uDatasetCrop` and the readout) and `releaseUvRegion` (flipped, the VR `uOverlayUvRegion`) are the map's rectangle. `overlayOptionsFromDataset` carries all of it; `Dataset.renderEncoding` stays unset, so the analysis tools and the generated colorbar stay off for these streams and the publisher's colorbar image remains the legend |
| `src/services/mapLayers.ts` | AYNI — the layer stack around a dataset, shared by both globes: the catalog client (`/api/layers`, once per session), the defaults by coverage (`defaultLayers`: a basemap under anything with transparency; white country borders on top of data that hides ≥70% of the Earth, measured on the first frame by `measureCoverage`, preferring a published white country-borders layer, then NASA reference features, then coastlines), and image loading shared across globes |
| `src/services/vrMapOverlays.ts` | AYNI — the layer stack's overlays on the immersive globe: one premultiplied, depth-tested, double-sided shell per overlay, a child of the globe mesh just outside vrBorders' shell, with a white / black / as-published tint |
| `src/services/vrOverlayOptions.ts` | AYNI — `vrOverlayOptionsFor`: the shared overlay bundle plus a release's map rectangle in the flipped UV space the THREE shaders sample in; kept apart from dashRelease so datasetOverlayOptions can import that module without a cycle |
| `src/services/dsaMetadata.ts` | The rest of a `.dsa` — creator, created/modified, product type, source link, keywords (encoding words dropped) and the completeness block — read leniently field by field (a descriptor with no time block still has a creator worth saying), and `describeStreamForDocent`, which turns it plus the axis and the frame on screen into the prompt lines Orbit answers metadata questions from: coverage and cadence, last update ("Forecast published" for a forecast), observed vs filled frame counts, and whether the frame on screen is observed data or a filled gap repeating an earlier frame. Registered by main.ts through docentService's `setStreamContextProvider`, which waits at most 1.5 s for it |
| `src/services/timelineTrackCanvas.ts` | Geometry, labels and drawing for a date track — the strip that says when the frame on the globe is, shared by the headset plane and the 2D transport panel so the two cannot disagree about where a tick falls or what a shaded span means. **Geometry is proportional, never absolute**: every inset, font size and bar position is a fraction of the canvas, which is what lets one implementation cover a 1200 x 220 VR strip and a 240 x 56 panel canvas, and the fractions were taken from the VR pixels so that surface did not move when the code was extracted (`timelineTrackCanvas.test.ts` pins its original numbers — an early rounded fraction put the bar 0.08 px off and the test caught it). Below 480 px it goes **compact**: no header row, no instant bubble, the axis ends labelled under the bar, because a transport strip cannot carry them and the surrounding panel already shows the current instant. Dates render in **UTC**, matching the axis and the MPD's `availabilityStartTime`; `uvToProgress` flips v because a Three plane's UV grows upward and a canvas's y does not, which is exactly how a hit test lands on the mirror image of the aim. Labelling is **distributed, not counted**: the tick ladder picks a count, but a count is not a fit — labels change width with the span ("Aug 18" against "Aug 2026"), the canvas changes with the panel, and a ladder that tops out can still return more ticks than the budget asked for (a ten-year axis falls through to its largest step and lands ten labels on a phone). So every candidate is measured, the axis’s ends are anchored, and a tick label is kept only where it clears everything already kept by a gap — a dropped label never drops its tick *line*, which is what keeps the rhythm while the text thins. The form follows the **step**, not the axis: a step shorter than a day prints the day and the time (a six-hour axis stepped every three hours otherwise says "Aug 18" four times), anything longer prints the day, and a span past a year prints the month and year. Verified by sweeping the real `drawTimelineTrack` over 3,552 width × height × span combinations for overlapping *and* repeated labels on a row — both zero. `SCRUB_SEEK_INTERVAL_MS` lives here too — one ration for both surfaces, because it is one decoder problem. |
| `src/services/vrTimelineTrack.ts` | The in-VR date track — a CanvasTexture strip that says *when* the frame on the globe is and lets the user move through the series, in the `vrTourControls` idiom (the interaction belongs to `vrInteraction`, the polling to `vrSession`). It draws the first and last frame's dates, a progress bar with tick marks and the playhead, the instant under the playhead (the question a curator asks in front of an audience), and **shades the spans the DSA flags** — the publisher's own note that 638 of 2,880 aircraft frames freeze the previous one is the only place a viewer could ever learn it. Dates render in **UTC** because the axis and the MPD's `availabilityStartTime` are declared that way and a museum device's timezone is nobody's own; ticks come from a ladder (`chooseTickIntervalMs`, about seven labels on any span) because a 30-day window and a 31-hour forecast need different steps, and the playhead label grows a time when the cadence is sub-daily. The bar's canvas rect is the single source of truth and the UV hit rect is derived from it, so drawing and hit-testing cannot drift; `progressAtUv` returns null off the bar, which is what leaves a ray that missed it to rotate the globe instead. |
| `src/ui/timelineTrackUI.ts` | The 2D transport's date track — the same strip, in `#playback-controls` above the scrubber, because the scrubber moves through two minutes of encoding that represent thirty days of data and nothing on that panel used to say which instant the sphere was showing. Only the DOM half: a canvas sized to its container at device pixel ratio, pointer drag, keyboard, and the ARIA slider semantics the scrubber already has (`aria-valuetext` carries the instant rather than a percentage — the instant is what is being chosen). **Seeks are rationed** through the shared constant while the drawn playhead follows the pointer at once, and the arrows step one frame of the axis with Home and End at its ends. Mounted by `main.ts` before the scrubber and fed from the same `timelineSnapshot()` the headset draws; lives in `src/ui/` so `check:i18n-strings` covers its one accessible name. |
| `src/services/vrSession.ts` | WebXR session lifecycle — requests `immersive-ar` or `immersive-vr`, wires renderer.xr, drives the per-frame loop, handles anchor persistence, falls back to `local` reference space if `local-floor` is unsupported. It also owns the in-VR control cluster's placement and lifetime: the HUD, the browse panel, the tour strip, the **date track** (`vrTimelineTrack`, polled from `getDatasetTimeline()` every frame, gated on the loading handover with the placement chrome, and answered by a host that reads the stream's `.dsa`) and the placement reticle/rail/button — each added hidden, positioned to billboard off the globe, and disposed on session end. |
...
| `src/services/vrHud.ts` | In-VR floating HUD — dataset title + play/pause + mute + Orbit mic + browse + exit-VR as a CanvasTexture panel with UV hit regions, plus a caption strip hung under it (a child mesh, so it follows the HUD and never takes a tap) carrying Orbit's side of a voice turn. The mic is drawn only when the host's `getVoiceState` returns a state, and its band goes back to the title otherwise. `hudActionAt` and `voiceCaption` are the pure layout and text rules, tested without a canvas |
...
| `src/services/vrDebugPanel.ts` | The in-AR diagnostic readout (`?vrDebug=1`) — a billboarded CanvasTexture panel floating in front of the camera, inert unless the URL asks for it. Exists because the AR failures it reports cannot be debugged the usual way: the phone has no devtools and a console line is worth nothing to the person holding it, while a panel is something they can photograph. Fields answer the questions a “the gestures do nothing” report actually raises, in order: `class`/`src`/`pad`/`domOv` (what the session and device reported — `pad=y` on a phone is the touch-position gamepad, not a controller), `hand`/`rotate`/`zoomUi` (the handheld-vs-headset decision, and whether the one-finger rotate layer and the zoom slider mounted on it), `img`/`load` (whether a dataset texture reached the scene and whether the loading scene is still up — what “there are two spheres” turns out to be), `loadT`/`ready`/`fade`/`shown` (the loop-clock seconds since the first frame and which of the handover's three steps — readiness, fade, reveal — have happened; the handover is driven from the render loop, never from timers, and hides the whole Earth stack via `vrScene.setEarthVisible` rather than the globe mesh alone, because the atmosphere shell is a scene-level sibling that a phone user could otherwise place and resize while the splash was still up), `cat`/`panels` (an empty catalog, and how many globes the 2D layout put in the scene), and `scale` (the number the suppressed pinch used to drive to maximum). Separate from `vrHud` because the HUD is the product surface with its own layout and tests. Re-texts at 1 Hz because `getDatasets()` allocates a fresh array per call; positions every frame because it is camera-locked. |
...
| `src/utils/vrCapability.ts` | Feature detection — `navigator.xr`, `immersive-vr`, `immersive-ar` support — plus `getInputArchetype()` (controller / screen / transient), `classifyXrDevice(ua, mode)` (UA-based bucket for `vr_session_started.device_class`) and `isHandheldArUserAgent(ua)` — the one-time, session-lifetime handheld-vs-headset decision every AR input gate in `vrSession` keys off. It replaced a "does any input source carry a gamepad" test that flapped on every phone tap, because Chrome on Android gives the screen-tap input source a gamepad whose axes are the touch position: the touch layer unmounted and remounted per touch, the pinch suppression dropped mid-tap, and the thumbstick-zoom path read the finger's Y as a stick push and resized the globe on contact. Quest / Pico / Vision Pro / HoloLens / Magic Leap UAs are matched first, so `android-ar` means exactly "an Android device that is not a headset" |
...
| `src/types/release-encoding.ts` | AYNI — the shape of a value-encoded DASH release's `valueEncoding` (`ReleaseEncoding`: kind, value range, code layout, data region, frame size, palette, alpha presentation, classes), as `services/dashRelease.ts` parses it. Kept in `types/` so `Dataset.vrValueEncoding` can name it without the types folder importing service code, which the Functions and CLI builds that share `types/` cannot compile |
