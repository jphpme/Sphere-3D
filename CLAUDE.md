...
| `src-tauri/capabilities/output.json` | Narrow capability scoped to the `output-*` window glob — IPC listen/emit, the window getters and setters an output drives **on itself**, and HTTPS fetch with localhost explicitly denied. `allow-destroy` sits beside `allow-close` and is **load-bearing, not belt-and-braces**: Tauri's `onCloseRequested` completes the close from JS by calling `destroy()` on the window itself when the handler does not `preventDefault()`, and that call is ACL-checked against the *output*. Granting `close` alone made every output unclosable — the manager's Remove, the operator's Alt+F4, everything — because the denial happens inside Tauri's own listener callback where nothing in this repo can see it, while `handle.close()` resolves normally on the manager side and `discard()` goes on to drop the record. Row gone, window alive, task manager the only way out. Found on hardware; it widens nothing `allow-close` did not already expose. No `core:default`, so no Tauri command `invoke` at all. It is defence in depth against a compromised output, **not** a restraint on the manager: Tauri checks window commands against the *calling* window, so putting the manager's grants here instead would deny every manager-initiated operation on an output |
