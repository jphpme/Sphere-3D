...
| `src/ui/vrTouchControls.ts` | Post-placement globe manipulation for handheld AR (the `screen` input class) — one finger drags the globe (it follows the finger in the camera-facing plane, resolved through the camera basis and the pixel→metre scale at the globe's distance), two fingers pinch to scale, two fingers twist to rotate past a 12° dead zone. DOM-driven because the XR path cannot express these on a phone: a screen tap's ray starts at the *device* and points through the touch point, so a drag reads as a direction change rather than a position, and two fingers arrive as two unrelated transient sources. Touches that begin on the zoom slider or the placement buttons are ignored here so they keep their own behaviour, and nothing is `preventDefault()`ed until a gesture is recognised — which is what leaves taps free to fall through to the XR path for the HUD, browse panel, tour strip and Place button. `vrInteraction` declines to grab the globe for this input class for the same reason. |
| `src/services/vrTouchGesture.ts` | Pure gesture maths behind `vrTouchControls` — the touch-point tracker (one finger → pixel deltas; two → separation ratio against the scale the pinch started from, plus twist accumulated per event so a full turn keeps counting), `pixelsToWorldScale` for pixel→metre at the globe's distance, `dragToWorld` for the camera-plane translation, and `twistAfterDeadzone`, which starts rotation from zero at the zone edge rather than jumping by it. Re-baselines whenever the finger set changes (a finger down, up, or a different finger replacing one) and reports nothing for that event, which is what stops the globe jumping when the second finger lands. Plain arithmetic, no DOM and no Three.js, so the gesture rules stay testable without AR hardware — the split `vrHeightControl` and `vrSpawn` already use. |
