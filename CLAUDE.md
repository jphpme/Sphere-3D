...
| `src/services/dashRelease.ts` | AYNI, immersive globe only — value-encoded DASH releases. The stream host publishes some streams as a `latest.json` pointer to an immutable `release.json` whose frames carry measurements in luma (`luma8-linear` / `luma8-log` / `luma8-classified`) above a calibration strip. `resolveDashRelease` follows the pointer to the current MPD, `.dsa` and encoding at every load; `parseReleaseEncoding` reads the publisher's code layout (no data below `nodataThresholdCode`, palette across `dataMinCode`..`dataMaxCode`, classified codes snapped to the nearest class) and presentation (`gradient` / `binary` / `opaque` alpha); `buildReleaseLut` is the exact 256-entry palette, and `releaseColorScale` restates it as one stop per code so the existing data-encoded VR branch reproduces it entry for entry; `releaseUvRegion` is the map's rectangle for the crop uniform (`uOverlayUvRegion` / `uSecUvRegion`). `vrOverlayOptionsFor` is the only consumer, from the VR context in main.ts. Nothing here sets `Dataset.renderEncoding`, and the rows are held out of the 2D catalog (`DataService.getImmersiveOnlyDatasets`), so the 2D map and the 3D browser globe are untouched |
