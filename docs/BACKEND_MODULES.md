...
| File | Responsibility |
|---|---|
| `functions/api/_lib/ai-models.ts` | The Workers AI model catalog behind Orbit's picker — one table of `{ id, cfId, tools?, nativeMultimodal?, legacyVision?, thinkingOff? }` from which the proxy's friendly-name map, the capability sets it routes on (`TOOL_CALLING_MODELS`, `NATIVE_MULTIMODAL_MODELS`, `LEGACY_VISION_MODELS`, `THINKING_DEFAULT_OFF_MODELS`), the thinking-off patch (`applyModelInputDefaults`, since Gemma 4, Qwen3 and GLM-4.7-Flash each read `enable_thinking` in their own chat template) and `/api/models`' list are all derived. It exists because those were two hand-synced lists: the endpoint's copy had outlived `llama-3.1-70b` and `llama-3.1-8b` (retired upstream, their catalog pages 404) and the proxy answered requests for both with the default model. Gemma 4 carries no `tools` on purpose — `0b0562bc` moved it off the tool set to keep its replies on the token-streaming path, which the tool shim's buffered call would replace; docs/ORBIT_MODEL_CATALOG_PLAN.md records the trade, the free-tier neuron costs and the paid-only models left out |
