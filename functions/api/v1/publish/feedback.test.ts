...
function setup() {
  ...
  // Timestamps must stay relative to now: both dashboards date-gate
  // their byDay/tag aggregations on `created_at >= now - days`, so
  // fixed dates silently age out of the default 30-day window.
  ...
  sqlite
    ...
    .run(
      ...
      '{"model":"llama-3.1-70b"}', 'what is ENSO?', 'ENSO is…', hoursAgo(30),
    )
  sqlite
    .prepare(
    ...
    .run('thumbs-down', '', 'msg-2', null, '["wrong"]', hoursAgo(25))
  sqlite
    .prepare(
      `INSERT INTO general_feedback (kind, message, contact, platform, screenshot, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
    )
    .run('bug', 'globe is upside down', 'a@b.c', 'web', 'data:image/jpeg;base64,abc123', hoursAgo(5))
  sqlite
    .prepare(
      `INSERT INTO general_feedback (kind, message, created_at)
       VALUES (?, ?, ?)`,
    )
    .run('feature', 'more datasets please', hoursAgo(1))
